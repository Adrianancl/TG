	...
	All the experiments were held without coolant, with velocities of 100 $m/min$ and 150 $m/min$ and a$_{p}$ = [0.2, 0.3, 0.4, 0.5] mm.
	...
	The design of the experiments are listed on the table \ref{tab:design}:
	...
	% #fazer figura da ferramenta com dimensões e peça tbm
	...
	\begin{equation} 
	\label{eq_energychip}
		\dot{Q}_{C}^{out} = c_{p}(T_{C}^{out})(T_{C}^{out} - T_{e})v_{f}t_{C}w
	\end{equation}
